			...
			ion_phys_addr_t *paddr, size_t *pa_len, void **vaddr)
{
	...
	*vaddr = ion_map_kernel(*client, *handle);
	...
	pr_debug("%s: mapped address = %pKK, size=%zd\n", __func__,
		*vaddr, bufsz);

	if (bufsz != 0) {
		pr_debug("%s: memset to 0 %pKK %zd\n", __func__, *vaddr, bufsz);
			...
			ion_phys_addr_t *paddr, size_t *pa_len, void **vaddr)
{
	...
	*handle = ion_import_dma_buf(*client, fd);
	pr_debug("%s: DMA Buf name=%s, fd=%d handle=%pKK\n", __func__,
							name, fd, *handle);
	if (IS_ERR_OR_NULL((void *) (*handle))) {
...
int msm_audio_ion_free(struct ion_client *client, struct ion_handle *handle)
{
	...
	if (msm_audio_ion_data.smmu_enabled) {
		...
		pr_debug("client=%pKK, domain=%pKK, domain_id=%d, group=%pKK",
		       ...
		       struct vm_area_struct *vma)
{
	...
	if (page) {
		...
		for_each_sg(table->sgl, sg, table->nents, i) {
			...
			pr_debug("vma=%pKK, addr=%x len=%ld vm_start=%x vm_end=%x vm_page_prot=%ld\n",
		...
		pr_debug("phys=%pKa len=%zd\n", &phys_addr, phys_len);
		pr_debug("vma=%pKK, vm_start=%x vm_end=%x vm_pgoff=%ld vm_page_prot=%ld\n",
...
{
	pr_debug("%s: client = %pKK smmu_enabled = %d\n", __func__,
...
static int msm_audio_ion_probe(struct platform_device *pdev)
{
	...
	if (smmu_enabled) {
		...
		if (IS_ERR_OR_NULL(msm_audio_ion_data.domain)) {
			pr_err("Failed to get domain data for group %pKK",
		...
		if (msm_audio_ion_data.domain_id < 0) {
			pr_err("Failed to get domain index for domain %pKK",
					msm_audio_ion_data.domain);
			goto fail_group;
		}
		pr_debug("domain=%pKK, domain_id=%d, group=%pKK",
			msm_audio_ion_data.domain,
			msm_audio_ion_data.domain_id, msm_audio_ion_data.group);
...
{
	pr_debug("%s: msm audio ion is unloaded, domain=%pKK, group=%pKK\n",
