...
static int _sendcache_pre(struct audio_client *ac)
{
	...
	if ((_depc_size == 0) || !_depc || (size == 0) ||
		...
		eagle_precache_err("%s: primary device %i cache index %i general error - cache size = %u, cache ptr = %pKK, offset = %u, size = %u, cmd = %i",
...
NT_MODE_GOTO:
	...
	if ((_depc_size == 0) || !_depc || (err != 0) || (size == 0) ||
		...
		eagle_postcache_err("%s: primary device %i cache index %i port_id 0x%X general error - cache size = %u, cache ptr = %pKK, offset = %u, size = %u, cmd = %i",
...
int msm_dts_eagle_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case DTS_EAGLE_IOCTL_SET_CACHE_SIZE: {
		...
		if (copy_from_user((void *)&size, (void *)arg, sizeof(size))) {
			eagle_ioctl_err("%s: error copying size (src:%pKK, tgt:%pKK, size:%zu)",
	...
	case DTS_EAGLE_IOCTL_GET_PARAM: {
		...
		if (copy_from_user((void *)&depd, (void *)arg, sizeof(depd))) {
			eagle_ioctl_err("%s: error copying dts_eagle_param_desc (src:%pKK, tgt:%pKK, size:%zu)",
	...
	case DTS_EAGLE_IOCTL_SET_PARAM: {
		...
		if (copy_from_user((void *)&_depc[offset],
				   (void *)(((char *)arg)+sizeof(depd)),
					depd.size)) {
			eagle_ioctl_err("%s: error copying param to cache (src:%pKK, tgt:%pKK, size:%u)",
	...
	case DTS_EAGLE_IOCTL_SET_CACHE_BLOCK: {
		...
		if (copy_from_user((void *)b_, (void *)arg, sizeof(b_))) {
			eagle_ioctl_err("%s: error copying cache block data (src:%pKK, tgt:%pKK, size:%zu)",
	...
	case DTS_EAGLE_IOCTL_SET_ACTIVE_DEVICE: {
		...
		if (copy_from_user((void *)data, (void *)arg, sizeof(data))) {
			eagle_ioctl_err("%s: error copying active device data (src:%pKK, tgt:%pKK, size:%zu)",
	...
	case DTS_EAGLE_IOCTL_GET_LICENSE: {
		...
		if (copy_from_user((void *)&target, (void *)arg,
				   sizeof(target))) {
			eagle_ioctl_err("%s: error reading license index. (src:%pKK, tgt:%pKK, size:%zu)",
	...
	case DTS_EAGLE_IOCTL_SET_LICENSE: {
		...
		if (copy_from_user(
				...
				target[1])) {
			eagle_ioctl_err("%s: error copying license to index %u, size %u (src:%pKK, tgt:%pKK, size:%u)",
					__func__, target[0], target[1],
	...
	case DTS_EAGLE_IOCTL_SET_VOLUME_COMMANDS: {
		...
		if (copy_from_user((void *)&spec, (void *)arg,
					sizeof(spec))) {
			eagle_ioctl_err("%s: error reading volume command specifier (src:%pKK, tgt:%pKK, size:%zu)",
		...
		if (spec & 0x80000000) {
			...
			if (copy_from_user((void *)&_vol_cmds_d[idx],
				...
				eagle_ioctl_err("%s: error reading volume command descriptor (src:%pKK, tgt:%pKK, size:%zu)",
			...
			if (copy_from_user((void *)_vol_cmds[idx],
				...
				eagle_ioctl_err("%s: error reading volume command string (src:%pKK, tgt:%pKK, size:%i)",
					__func__, ((char *)arg) + (sizeof(int) +
					sizeof(struct vol_cmds_d)),
					_vol_cmds[idx], size);
				return -EFAULT;
