...
static int32_t afe_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == RESET_EVENTS) {
		pr_debug("%s: reset event = %d %d apr[%pKK]\n",
			__func__,
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		if ((data->payload_size < sizeof(this_afe.calib_data))
			...
			pr_err("%s: Error: size %d payload %pKK token %d\n",
...
static int afe_send_cal_block(u16 port_id, struct cal_block_data *cal_block)
{
	...
	pr_debug("%s: AFE cal sent for device port = 0x%x, cal size = %zd, cal addr = 0x%pKa\n",
		__func__, port_id,
...
static int afe_send_custom_topology_block(struct cal_block_data *cal_block)
{
	...
	pr_debug("%s:cmd_id:0x%x calsize:%zd memmap_hdl:0x%x caladdr:0x%pKa",
...
int afe_cmd_memory_map(phys_addr_t dma_addr_p, u32 dma_buf_sz)
{
	...
	pr_debug("%s: dma_addr_p 0x%pKa , size %d\n", __func__,
...
int afe_map_rtac_block(struct rtac_cal_block_data *cal_block)
{
	...
	result = afe_cmd_memory_map(cal_block->cal_data.paddr,
		cal_block->map_data.map_size);
	if (result < 0) {
		pr_err("%s: afe_cmd_memory_map failed for addr = 0x%pKa, size = %d, err %d\n",
