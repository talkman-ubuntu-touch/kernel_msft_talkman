			       ...
			       bool detectfailure)
{
	...
	pr_debug("%s: addr %pKa, size %d, handle 0x%x\n", __func__,
				    ...
				    uint32_t *mmap_p)
{
	...
	pr_debug("%s: dma_addr_p 0x%pKa, dma_buf_sz %d, mmap_p 0x%pKK, session %d\n",
			      ...
			      bool allocate_module_data)
{
	...
	pr_debug("%s:Snd Model len = %zd cal size %zd phys addr %pKa", __func__,
	...
	pr_debug("%s: Copy cal start virt_addr %pKK phy_addr %pKa\n"
			 "Offset cal virtual Addr %pKK\n", __func__,
...
int q6lsm_lab_control(struct lsm_client *client, u32 enable)
{
	...
	if (!client) {
		pr_err("%s: invalid param client %pKK\n", __func__, client);
		return -EINVAL;
	}
...
int q6lsm_read(struct lsm_client *client, struct lsm_cmd_read *read)
{
	int rc = 0;
	if (!client || !read) {
		pr_err("%s: Invalid params client %pKK read %pKK\n", __func__,
			client, read);
		return -EINVAL;
	}
...
int q6lsm_lab_buffer_alloc(struct lsm_client *client, bool alloc)
{
	...
	if (alloc) {
		...
		if (ret) {
			...
			pr_debug("%s: Memory map handle %x phys %pKa size %d\n",
				__func__,
				client->lab_buffer[0].mem_map_handle,
