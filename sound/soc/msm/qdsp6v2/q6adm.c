		      ...
		      void *data, uint32_t size)
{
	...
	if ((size == 0) || !data) {
		pr_err("DTS_EAGLE_ADM: %s - invalid size %u or pointer %pKK.\n",
			__func__, size, data);
		return -EINVAL;
	}
...
static void remap_cal_data(struct cal_block_data *cal_block, int cal_index)
{
	...
	if ((cal_block->map_data.map_size > 0) &&
		...
		if (ret < 0) {
			...
			pr_debug("%s: ADM mmap did not work! addr = 0x%pKa, size = %zd ret %d\n",
				__func__,
...
static void send_adm_custom_topology(void)
{
	...
	atomic_set(&this_adm.adm_stat, 0);
	pr_debug("%s: Sending ADM_CMD_ADD_TOPOLOGIES payload = 0x%pKa, size = %d\n",
			      ...
			      int app_type, int acdb_id, int sample_rate)
{
	...
	atomic_set(&this_adm.copp.stat[port_idx][copp_idx], 0);
	pr_debug("%s: Sending SET_PARAMS payload = 0x%pKa, size = %d\n",
		__func__, &cal_block->cal_data.paddr,
		adm_params.payload_size);
	...
	if (result < 0) {
		...
		pr_debug("%s: Set params failed port = 0x%x payload = 0x%pKa result %d\n",
	...
	if (!result) {
		pr_err("%s: Set params timed out port = 0x%x\n",
				__func__, port_id);
		pr_debug("%s: Set params timed out port = 0x%x, payload = 0x%pKa\n",
	     ...
	     int perf_mode, uint16_t bit_width, int app_type, int acdb_id)
{
	...
	if ((topology == SRS_TRUMEDIA_TOPOLOGY_ID) &&
		...
		if (res < 0) {
			pr_err("%s: SRS adm_memory_map_regions failed ! addr = 0x%pKK, size = %d\n",
...
int adm_map_rtac_block(struct rtac_cal_block_data *cal_block)
{
	...
	if (result < 0) {
		...
		pr_debug("%s: RTAC mmap did not work! addr = 0x%pKa, size = %d\n",
			__func__,
			&cal_block->cal_data.paddr,
