...
static struct voice_data *voice_get_session(u32 session_id)
{
	...
	pr_debug("%s:session_id 0x%x session handle %pKK\n",
				...
				uint32_t session_id)
{
	...
	result = voice_map_memory_physical_cmd(v,
		...
		VOC_CAL_MEM_MAP_TOKEN);
	if (result < 0) {
		pr_err("%s: Mmap did not work! addr = 0x%pKa, size = %zd\n",
			__func__,
			&cal_block->cal_data.paddr,
			cal_block->map_data.map_size);
			   ...
			   uint32_t session_id)
{
	...
	if ((cal_block->map_data.map_size > 0) &&
		...
		pr_debug("%s:  Cal block 0x%pKa, size %zd already mapped. Q6 map handle = %d\n",
...
int voc_map_rtac_block(struct rtac_cal_block_data *cal_block)
{
	...
	if (!is_rtac_memory_allocated()) {
		...
		if (result < 0) {
			pr_err("%s: RTAC alloc mem map table did not work! addr = 0x%pKa, size = %d\n",
				__func__,
				&cal_block->cal_data.paddr,
				cal_block->map_data.map_size);
	...
	result = voice_map_memory_physical_cmd(v,
		...
		VOC_RTAC_MEM_MAP_TOKEN);
	if (result < 0) {
		pr_err("%s: RTAC mmap did not work! addr = 0x%pKa, size = %d\n",
			__func__,
			&cal_block->cal_data.paddr,
			cal_block->map_data.map_size);
...
static int voice_send_cvs_packet_exchange_config_cmd(struct voice_data *v)
{
	...
	pr_debug("%s: dec buf: add %pKa, size %d, enc buf: add %pKa, size %d\n",
		__func__,
		&dec_buf,
...
int voc_start_record(uint32_t port_id, uint32_t set, uint32_t session_id)
{
	...
	while (voice_itr_get_next_session(&itr, &v)) {
		...
		pr_debug("%s: port_id: %d, set: %d, v: %pKK\n",
...
static int voice_alloc_oob_shared_mem(void)
{
	...
	pr_debug("%s buf[0].data:[%pKK], buf[0].phys:[%pKa], &buf[0].phys:[%pKK],\n",
		 __func__,
		(void *)v->shmem_info.sh_buf.buf[0].data,
		&v->shmem_info.sh_buf.buf[0].phys,
		(void *)&v->shmem_info.sh_buf.buf[0].phys);
	pr_debug("%s: buf[1].data:[%pKK], buf[1].phys[%pKa], &buf[1].phys[%pKK]\n",
		__func__,
